...
public class Graph {
    ...
    public double calculateWeight(int distance, int traffic, int scenery, int attractions) {
        double weight = (traffic*scenery*attractions) + distance;
        return weight;
    }

    public Map<String, Double> dijkstra(Vertex start) {
        Map<String, Double> Table = new HashMap<String, Double>();
        if(!_vertices.containsKey(start.getLabel())){
            Table.put("F",0.0);
            return Table;
        }
        start.setDistanceFromSource(0);
        List<Vertex> Known = new ArrayList<Vertex>();
        BinaryHeap<Vertex,Double> VPQ = new MinBinaryHeap<Vertex,Double>();
        VPQ.enqueue(start, 0.0);

        while(!(VPQ.size() == 0)){
            Vertex V = VPQ.dequeue();
            Vertex Current = _vertices.get(V.getLabel());
            if(!Known.contains(Current)){
                Known.add(Current);
                Table.put(Current.getLabel(),Current.getDistanceFromSource());
                for (Edge a : Current.getEdges()){
                    if(Current.getDistanceFromSource()+a.getWeight() < a.getDestination().getDistanceFromSource()||a.getDestination().getDistanceFromSource() == 0){
                        a.getDestination().setDistanceFromSource((Current.getDistanceFromSource()+a.getWeight()));
                        VPQ.enqueue(a.getDestination(),a.getDestination().getDistanceFromSource());
                    }
